...
public class SignatureGenerator {
    ...
    public static String generateSignature(String secret, String payload) {
        Mac hmacSha256;
        try {
            hmacSha256 = Mac.getInstance(PoloApiConstants.SIGNATURE_METHOD_VALUE);
//            这行声明了一个Mac（Message Authentication Code）类型的变量hmacSha256，但没有初始化。Mac类是用于生成消息认证码的类。

            SecretKeySpec secKey = new SecretKeySpec(secret.getBytes(StandardCharsets.UTF_8), PoloApiConstants.SIGNATURE_METHOD_VALUE);
